...
object OpenSslSupport {
  def apply(sslFactory: PipelineContext => SSL,
            ...
            client: Boolean = true): PipelineStage =
    new PipelineStage {
      def showError() {
      ...
      final class SslPipelines(context: PipelineContext, commandPL: CPL, eventPL: EPL) extends CopyingBIOImpl with Pipelines {
        ...
        def write(buffer: Array[Byte]): Int = {
          ...
          commandPL(IOConnection.Send(ByteBuffer.wrap(buffer)))
          buffer.length
        }
        ...
        val eventPipeline: EPL = {
          ...
          case c@IOConnection.Closed(_, _) =>
        ...
        @tailrec private[this] def checkPendingSSLOutput(direct: DirectBuffer) {
          ...
          if (pendingBytes > 0) {
            ...
            if (read > 0) {
              ...
              eventPL(IOConnection.Received(context.connection, buffer))
        ...
        def withTempBuf(f: DirectBuffer => Unit) {
          ...
          try f(tempBuf)
            ...
            case e: RuntimeException =>
              context.log.error(e, "Closing encrypted connection to {} due to {}", context.connection.remoteAddress, e)
              commandPL(IOConnection.Close(ConnectionCloseReasons.ProtocolError(e.toString)))
          }
