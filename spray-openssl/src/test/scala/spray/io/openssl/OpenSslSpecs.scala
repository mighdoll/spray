...
class OpenSslSpecs extends TestKitBase with Specification {
  ...
  "OpenSslSupport" should {
    ...
    "support client mode" in {
      "run a full cycle" in withEstablishedConnection() { setup =>
        ...
        javaSslServer.commander.expectMsgType[IOConnection.Received].buffer.remaining() must be_==(8)
        ...
        openSslClient.commander.expectMsgType[IOConnection.Received].buffer.remaining() must be_==(4)
      }
      "make sure big buffers are written properly" in withEstablishedConnection() { setup =>
        ...
        @tailrec def receiveAll(remaining: Int): Unit =
          if (remaining > 0) {
            transferToServer() must be_==(Seq(EncryptedData))
            val received = javaSslServer.commander.expectMsgType[IOConnection.Received]
  ...
  case class StageTestSetup(commander: TestProbe, stageActor: ActorRef, stateMachine: SimpleTlsStateMachine) {
    def transferSSLMessageTo(other: StageTestSetup): Seq[SSLMessage] = {
      val data = commander.expectMsgType[IOConnection.Send]
    ...
    def sendData(bytes: Array[Byte]): Unit =
      commander.send(stageActor, IOConnection.Send(ByteBuffer.wrap(bytes)))
  }
  ...
  def reverse(command: IOConnection.Send): IOConnection.Received = command match {
    case IOConnection.Send(Seq(one), _) => IOConnection.Received(null, one)
  }
  def reverse(event: IOConnection.Received): IOConnection.Send =
    IOConnection.Send(event.buffer, None)
  ...
  def createStageActor(stage: PipelineStage, handler: ActorRef): ActorRef = {
    ...
    class StageActor extends Actor with ActorLogging { actor =>
        ...
        val pipes =
          stage(new PipelineContext {
            ...
            val connection: Connection = new Connection {
              ...
              override def remoteAddress: Option[InetSocketAddress] = None
            }
          }, handler !, handler !)
      ...
      override def postStop() {
        stageEventPL(IOConnection.Closed(null, PeerClosed))
      }
    }
