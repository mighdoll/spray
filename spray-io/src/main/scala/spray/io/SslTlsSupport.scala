...
case class ServerSSLEngineProvider(stage: PipelineStage) extends StageProvider
object ServerSSLEngineProvider extends SSLEngineProviderCompanion {
  ...
  implicit def fromFunc(f: PipelineContext => SSLEngine): Self =
    new ServerSSLEngineProvider(SslTlsSupport(f, encryptIfUntagged = true))
}

case class ClientSSLEngineProvider(stage: PipelineStage) extends StageProvider
object ClientSSLEngineProvider extends SSLEngineProviderCompanion {
  ...
  implicit def fromFunc(f: PipelineContext => SSLEngine): Self =
    new ClientSSLEngineProvider(SslTlsSupport(f, encryptIfUntagged = false))
}
